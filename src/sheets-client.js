...
/**
 * Reads categories from the 'llamada_appscript' sheet
 */
export async function readCategories(env) {
    if (!env.GOOGLE_PRIVATE_KEY) throw new Error('MISSING_SECRET: GOOGLE_PRIVATE_KEY no está configurado.');
    if (!env.GOOGLE_CLIENT_EMAIL) throw new Error('MISSING_VAR: GOOGLE_CLIENT_EMAIL no está configurado.');

    const token = await getAccessToken(env.GOOGLE_CLIENT_EMAIL, env.GOOGLE_PRIVATE_KEY);

    // Fetch the llamada_appscript sheet
    ...
    // Find the category column (adjust based on actual sheet structure)
    const categoryIdx = headers.indexOf('categoria') !== -1
        ? headers.indexOf('categoria')
        : headers.indexOf('Categoria') !== -1
            ? headers.indexOf('Categoria')
            : 0; // Default to first column if not found

    // Extract unique categories
    const categories = [...new Set(
        dataRows
            .map(row => String(row[categoryIdx] || '').trim())
            .filter(cat => cat && cat !== '')
    )];
